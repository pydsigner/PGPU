...
class BasicChronograph(tk.Label):
    ...
    def update(self):
        ...
        if self.on:
            ...
            hrs = str(self.time_expired.days * 24)
            mins = str(self.time_expired.seconds // 60)
            secs = str(self.time_expired.seconds % 60)
            hundrths = str(self.time_expired.microseconds // 10000)
            self['text'] = '%s:%s:%s.%s' % (hrs.zfill(2), mins.zfill(2), secs.zfill(2), hundrths.zfill(2))
        else:
