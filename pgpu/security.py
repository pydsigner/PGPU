...
#class PD1(object):
#	'''Don't use this, use another hasher instead. 
#	This hasher will be removed by 0.3.0.'''
#	def __init__(self):
#		warnings.warn('PD1 is not useful, and will be removed by 0.3.0!')
#	def encode(self, s):
#		l = len(s)
#		res = [str(ord(c) * l) for c in s]
#		return ''.join(res)
# __all__.append('PD1')
# encoder_classes['pd1'] = PD1
...
class PD4(object):
	...
	but you may wish to look at PD5(), PD7(), or PD8() for checksums. NOTE: 
	This is VERY slow!
	AUTHORS:
	v0.2.9+			--> pydsigner'''
	decimal.getcontext().prec = 15
...
class RD(object):
	'''Seeds the random module with the object to be hashed; returns the 
	str()ed result of random.randint(self.small, self.big). Possible weaknesses: 
	Does random return the same numbers on all platforms? Will threading break 
	this?
	AUTHORS:
	v0.4.7+			--> pydsigner'''
	small = 0
